      ...
      <section className="specialty-example-grid">
        {content.items.map((item,index) => (
          <article key={item.title}>
            <div className="specialty-example-image" role="img" aria-label={"Exemplo de " + item.title} style={{ backgroundPosition: item.position, backgroundSize:item.imageUrl?"cover":"300% auto", backgroundImage:`linear-gradient(0deg,rgba(0,0,0,.55),transparent 56%),url(${item.imageUrl||"/instagram-reference.png"})` }}>
