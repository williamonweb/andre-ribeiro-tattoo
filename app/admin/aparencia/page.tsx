...
import { LogoBrandEditor } from "@/components/logo-brand-editor";
...
export default async function Page({searchParams}:{searchParams:Promise<{saved?:string}>}){
  ...
  <form action={saveAppearance} className="admin-card appearance-form">
    ...
    <div className="appearance-preview" style={{backgroundColor:data.backgroundColor,borderColor:data.goldColor}}>{data.logoImageUrl?<img className="preview-logo-image" src={data.logoImageUrl} alt="Logo atual"/>:<div className="preview-brand" style={{color:data.goldBrightColor,borderColor:data.goldColor}}>{data.logoInitials}</div>}<div><strong>Prévia da marca</strong><span style={{color:data.goldColor}}>{data.brandSuffix}</span></div><Palette size={22}/></div>
