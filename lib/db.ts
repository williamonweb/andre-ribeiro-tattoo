...
export function ensureDatabase(){if(!ready){const sql=database();ready=Promise.all([
  ...
  sql`CREATE TABLE IF NOT EXISTS testimonial_request (id BIGSERIAL PRIMARY KEY, name TEXT NOT NULL, city TEXT, tattoo_style TEXT, message TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pendente', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), reviewed_at TIMESTAMPTZ)`,
